...
func (s *AccountModel) Create(acc *Account) error {
	...
	(username, first_name, last_name, email_addr, about, encrypted_password, create_at)
	values ($1, $2, $3, $4, $5, $6))`

	_, err := s.DB.Exec(query, acc.Username, acc.FirstName, acc.LastName, acc.EmailAddr, acc.EncryptedPassword, acc.CreatedAt)
	return err
}
